...
- configuration-driven monitoring of 15 repositories;
...
`config/cross-spec-pressure-tests.yaml` declares eight DTG composition seams, all now runnable through RAHP Toolkit. The Portfolio Monitor points DTG operators to the dedicated `dtg-cross-spec-pressure-test.yml` launcher, while also recording the generic profile-driven workflow as the ecosystem-neutral extension point. The Portfolio Monitor exposes readiness, evidence grade and relationships; RAHP Toolkit owns execution, evidence and durable review issues through the optional `profiles/dtg/` pack. Upstream issue filing remains a human governance decision after RAHP findings are triaged.
