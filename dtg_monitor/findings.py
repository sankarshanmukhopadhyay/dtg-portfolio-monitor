...
def validate_dispositions(dispositions: list[dict[str, Any]]) -> None:
    ...
    for index, item in enumerate(dispositions):
        ...
        if not isinstance(fingerprint, str) or len(fingerprint) != 24:
            raise ValueError(f"{prefix}.fingerprint must be a 24-character finding fingerprint")
        if fingerprint in seen:
            raise ValueError(f"duplicate finding disposition for {fingerprint}")
        seen.add(fingerprint)
        if item.get("state") not in DISPOSITION_STATES:
            raise ValueError(f"{prefix}.state must be one of {sorted(DISPOSITION_STATES)}")
        authority = item.get("authority")
        if not isinstance(authority, dict) or not authority.get("actor") or not authority.get("basis"):
            raise ValueError(f"{prefix}.authority requires actor and basis")
        disposition = item.get("disposition")
        if not isinstance(disposition, dict) or not disposition.get("reason") or not disposition.get("recorded_at"):
            raise ValueError(f"{prefix}.disposition requires reason and recorded_at")
        try:
...
def apply_dispositions(findings: list[dict[str, Any]], dispositions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    by_fingerprint = {item["fingerprint"]: item for item in dispositions}
    result: list[dict[str, Any]] = []
    for finding in findings:
        updated = dict(finding)
        decision = by_fingerprint.get(finding["fingerprint"])
        if decision:
            updated["state"] = decision["state"]
            updated["review_status"] = decision.get("review_status", "no-action")
            updated["authority"] = decision["authority"]
            updated["disposition"] = decision["disposition"]
            updated["related_findings"] = decision.get("related_findings", [])
            updated["successor_finding"] = decision.get("successor_finding")
